...
test("/patchSponsorlvl/:sponsorId - Patch sponsor level", async () => {
  ...
  const patchSponsorlvlRes = await fetch(`${SERVER_URL}/patchSponsorlvl/${fakeData.sponsors[0].id}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  expect(patchSponsorlvlBody.response).toBe(`Document with ID ${fakeData.sponsors[0].id} patched.`);
});

test("/deleteSponsor/:sponsorId - Delete sponsor", async () => {
  const sponsorId = fakeData.sponsors[0].id;
...
test("/getWashroom/:washroomId - Get washroom by ID", async () => {
  const washroomId = fakeData.washrooms[0].id;
...
test("/patchWashroom/:washroomId - Patch washroom by ID", async () => {
  ...
  const patchWashroomRes = await fetch(`${SERVER_URL}/patchWashroom/${fakeData.washrooms[0].id}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  expect(patchWashroomBody.response).toBe(`Document with ID ${fakeData.washrooms[0].id} patched.`);
});

test("/getUser/:userId - Get user by ID", async () => {
  const userId = fakeData.users[0].id;
...
test("/patchUser/:userId - Patch user by ID", async () => {
  ...
  const userId = fakeData.users[0].id; // Assuming there's at least one user in the fakeData
...
test("/patchRequestStatus/:washroomId - Patch request status", async () => {
  const washroomId = fakeData.requested[0].id; // Assuming there's at least one requested washroom in the fakeData
